...
def fetch_alpha_data():
    ...
    # Lógica: Quando Preço < Média -> Z é Positivo (Bottom)
    # Quando Preço > Média -> Z é Negativo (Top)
...
try:
    ...
    # CONSTRUÇÃO DO PLOT (REPLICA EXATA)
    fig = make_subplots(
        rows=2, cols=1, 
        shared_xaxes=True, 
    ...
    # 2. PAINEL Z-SCORE
    fig.add_trace(
        go.Scatter(x=data.index, y=data['z_score'], name="Cycle Indicator", line=dict(color='#888', width=1.2)),
        row=2, col=1
    )

    # LINHAS TRACEJADAS DE REFERÊNCIA
    # Upside (Blue Zone: -1 a -3 SD)
    ...
    # PREENCHIMENTO DO INDICADOR (APENAS NAS ZONAS)
    # Coloração Blue (Upside / Overbought)
    ...
    # Coloração Aqua (Downside / Oversold)
    fig.add_trace(go.Scatter(x=data.index, y=[1.0]*len(data), line=dict(width=0), showlegend=False), row=2, col=1)
    fig.add_trace(go.Scatter(
        x=data.index, 
        y=np.where(data['z_score'] >= 1.0, data['z_score'], 1.0),
    ...
    # Layout Mestre com Eixo Invertido
    fig.update_layout(
    ...
    # AQUI ESTÁ O SEGREDO: autorange='reversed'
    fig.update_yaxes(
        row=2, col=1, 
        gridcolor="#222", 
        title="Cycle Stress (SD)", 
        autorange='reversed', # Inverte o eixo visualmente
        range=[-4, 4]
    )
