...
try:
    ...
    # Título Centralizado em Estilo "Italiano"
    st.markdown("<h1 style='text-align: center; color: #00FBFF; font-family: serif;'>✦ 𝓐𝓵𝓹𝓱𝓪 𝓒𝔂𝓬𝓵𝓮 𝓓𝓮𝓿𝓲𝓪𝓽𝓲𝓸𝓷 ✦</h1>", unsafe_allow_html=True)
    st.markdown("<p style='text-align: center; color: #888; margin-bottom: 30px;'>Institutional Macro Cycle Monitor</p>", unsafe_allow_html=True)
    ...
    with c3:
        ...
        if last_z >= 2.0: 
            ...
            status = "🔴 SELL (TOP)"
            s_color = "#3D5AFE"
        st.markdown(f"<h1 style='text-align: right; color: {s_color}; margin-top: 10px;'>{status}</h1>", unsafe_allow_html=True)

    # CONSTRUÇÃO DO PLOT ACD
    fig = make_subplots(
        rows=2, cols=1, 
        shared_xaxes=True, 
    ...
    fig.update_yaxes(row=2, col=1, gridcolor="#222", title="ACD Stress (SD)", autorange='reversed', range=[-4, 4])
