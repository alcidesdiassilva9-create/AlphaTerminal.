...
if not data.empty:
    ...
    # Matriz de Sentimento Ajustada à Nova Escala
...
@@ -53,46 +58,34 @@ def fetch_anupl_calibrated():
    ...
    # Linhas de Escala Solicitadas: 3, 2, 1 (Oversold) | -1, -2, -3 (Overbought)
    # Lado Overbought (Blue)
    fig.add_hline(y=-1.0, line=dict(color="rgba(61, 90, 254, 0.3)", width=1, dash="dot"), row=2, col=1)
    ...
    # Lado Oversold (Aqua)
    fig.add_hline(y=1.0, line=dict(color="rgba(0, 251, 255, 0.3)", width=1, dash="dot"), row=2, col=1)
    ...
    # Configuração de Eixos e Escala Fixa
